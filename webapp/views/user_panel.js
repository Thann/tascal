...
module.exports = Backbone.View.extend({
	...
	template: _.template(`
		<div class="user panel panel-default">
			<div class="panel-heading" data-toggle="collapse" data-target=".user .panel-collapse">
				<div class="panel-title"><%= user.get('username') %></div>
			</div>
	...
	initialize: function() {
		// console.log("UUU", Tascal.Router.args, Tascal.User.get('username'))
		const username = Tascal.Router.args[0];
		if (username === Tascal.User.get('username')) {
		...
		const me = this.user.id === Tascal.User.id;
		this.scope = {
			me,
			user: this.user,
			logs: this.logs,
	...
	render: function() {
		if (Tascal.Router.args[0] !== this.user.get('username'))
	...
	delUser: function() {
		if (confirm('Are you sure you want to delete '+
			this.user.get('username')+'?')) {
			this.user.destroy();
			Tascal.Router.navigate('/admin', {trigger: true});
		}
	},
});
